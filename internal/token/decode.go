...
// NewPGPDecrypter takes a passphrase and returns a GPG based Decypter decode function
func NewPGPDecrypter(passphrase string) Decode {
	return func(body string) (string, error) {
		return pgpDecryptToken(body, passphrase)
	}
}
...
func pgpDecryptToken(base64Body, passphrase string) (string, error) {
